...
namespace PyTrain.DesktopClient.Common.Services;

public static class DisplayCoordinateConverter
{
  /// <summary>
  /// Returns a capture-pixel coordinate relative to the display's own top-left corner (0, 0).
  ...
  /// </summary>
  public static PhysicalPoint DisplayPercentToCapturePoint(double percentOfDisplayX, double percentOfDisplayY, DisplayInfo display)
  {
    var clampedX = Math.Clamp(percentOfDisplayX, 0, 1);
    var clampedY = Math.Clamp(percentOfDisplayY, 0, 1);

    var maxX = Math.Max(0, display.CapturePixelSize.Width - 1);
    var maxY = Math.Max(0, display.CapturePixelSize.Height - 1);
  ...
  public static LogicalPoint DisplayPercentToLayoutPoint(double percentOfDisplayX, double percentOfDisplayY, DisplayInfo display)
  {
    var clampedX = Math.Clamp(percentOfDisplayX, 0, 1);
    var clampedY = Math.Clamp(percentOfDisplayY, 0, 1);

    var bounds = display.LayoutBounds;
