...
public class DesktopClientWatcherWinTests
{
  ...
  public async Task RunIteration_WhenInstanceIdIsMissing_OmitsInstanceIdArgument()
  {
    ...
    Assert.Equal("\"C:\\PyTrain\\DesktopClient\\PyTrain.DesktopClient.exe\"", launchedCommand);
  }
  ...
  private DesktopClientWatcherWin CreateWatcher(string instanceId = "instance-1")
  {
    ...
    _environment.SetupGet(x => x.StartupDirectory).Returns("C:\\PyTrain");
    ...
    _fileSystem
      ...
      .Returns(true);
    _pathProvider.Setup(x => x.GetDesktopExecutablePath()).Returns("C:\\PyTrain\\DesktopClient\\PyTrain.DesktopClient.exe");
    _waiter
      .Setup(x => x.WaitFor(
        It.IsAny<Func<bool>>(),
        It.IsAny<TimeSpan?>(),
