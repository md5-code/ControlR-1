...
internal sealed class UiBrowserSession : IAsyncDisposable
{
  ...
  {
    var agentProjectPath = GetProjectPath("PyTrain.Agent", "PyTrain.Agent.csproj");
  ...
  {
    var desktopClientProjectPath = GetProjectPath("PyTrain.DesktopClient", "PyTrain.DesktopClient.csproj");
    var outputLog = new ConcurrentQueue<string>();
    ...
    UiTestDatabaseSession databaseSession)
  {
    ...
    var serverProjectPath = GetProjectPath("PyTrain.Web.Server", "PyTrain.Web.Server.csproj");
    var outputLog = new ConcurrentQueue<string>();
    ...
    var processSession = StartDotNetProcess(
      ...
      environmentVariables: new Dictionary<string, string>
      {
        ...
        ["PyTrain_POSTGRES_HOST"] = databaseSession.Host,
        ["PyTrain_POSTGRES_PORT"] = databaseSession.Port.ToString(),
        ["PyTrain_POSTGRES_DB"] = databaseSession.DatabaseName,
      });
...
internal sealed class UiTestDatabaseSession : IAsyncDisposable
{
  ...
  {
    var databaseName = $"pytrain_ui_{Guid.NewGuid():N}";
