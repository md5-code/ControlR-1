...
namespace PyTrain.Web.Server.Tests;

public partial class PyTrainApiContractSyncTests
{
  [Fact]
  public void HttpConstants_AreExplicitlyMappedToIPyTrainApiSubClients()
  {
    var endpointPropertyMap = CreateEndpointPropertyMap();
    var constantValues = GetHttpConstantValues();

    var actualPropertyNames = typeof(PyTrain.ApiClient.IPyTrainApi)
  ...
  [Fact]
  public void OpenApiApiPaths_MapToKnownIPyTrainApiSubClients()
  {
    var apiPaths = LoadOpenApiApiPaths();
    var constantValues = GetHttpConstantValues();
    var endpointPropertyMap = CreateEndpointPropertyMap();
    ...
    foreach (var apiPath in apiPaths)
    {
      ...
      Assert.True(
        ...
        $"Mapped sub-client '{propertyName}' for route '{matchedConstant}' is not present on IPyTrainApi.");
    }
  }

  private static Dictionary<string, string> CreateEndpointPropertyMap()
  {
    var propertyNames = typeof(PyTrain.ApiClient.IPyTrainApi)
      .GetProperties(BindingFlags.Instance | BindingFlags.Public)
      .Select(property => property.Name)
      .ToHashSet(StringComparer.Ordinal);
    ...
    foreach (var field in constantFields)
    {
      ...
      Assert.True(
        ...
        $"HttpConstants field '{field.Name}' inferred IPyTrainApi property '{propertyName}', but that property was not found.");
  ...
  private static string FindRepositoryRoot()
  {
    ...
    {
      var candidatePath = Path.Combine(current.FullName, "PyTrain.slnx");
    ...
    throw new DirectoryNotFoundException("Could not locate repository root containing PyTrain.slnx.");
  }
  ...
  private static HashSet<string> LoadClientRouteTemplates(IReadOnlyCollection<string> constantValues)
  {
    ...
    var apiClientDirectory = Path.Combine(repositoryRoot, "PyTrain.ApiClient", "Implementations");
    var sourceFiles = Directory
      .EnumerateFiles(apiClientDirectory, "PyTrainApi*.cs", SearchOption.TopDirectoryOnly)
      .Where(path => !path.EndsWith("PyTrainApi.cs", StringComparison.OrdinalIgnoreCase))
      .ToArray();
  ...
  {
    var repositoryRoot = FindRepositoryRoot();
    var openApiPath = Path.Combine(repositoryRoot, "PyTrain.Web.Server", "PyTrain.Web.Server.json");
