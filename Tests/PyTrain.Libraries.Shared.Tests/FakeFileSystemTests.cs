...
namespace PyTrain.Libraries.Shared.Tests;
...
public class FakeFileSystemTests
{
  ...
  public void JoinPaths_RemovesDuplicateSeparatorsWhenSegmentsContainLeadingTrailingSeparators()
  {
    ...
    var path = fileSystem.JoinPaths('/', "/tmp/", "/PyTrain_Update/", "installer");

    Assert.Equal("/tmp/PyTrain_Update/installer", path);
  }
  ...
  {
    var fileSystem = new FakeFileSystem();
    fileSystem.SetResolvedFilePath("pytrain", "/usr/bin/pytrain");

    var result = await fileSystem.ResolveFilePath("pytrain");
    var missing = await fileSystem.ResolveFilePath("missing");

    Assert.True(result.IsSuccess);
    Assert.Equal("/usr/bin/pytrain", result.Value);
