...
namespace PyTrain.ApiClientExample;

internal class ExampleService(
  IServiceProvider serviceProvider,
  IHostApplicationLifetime hostLifetime,
  IOptionsMonitor<PyTrainApiClientOptions> optionsMonitor,
  ILogger<ExampleService> logger): BackgroundService
{
  ...
  private readonly IOptionsMonitor<PyTrainApiClientOptions> _optionsMonitor = optionsMonitor;
  ...
  private async Task RetrieveDevices(IPyTrainApi client, CancellationToken cancellationToken)
  {
  ...
  {
    _logger.LogInformation("Retrieving devices using IPyTrainApiClientFactory.");

    var client = _serviceProvider.GetRequiredService<IPyTrainApi>();
    await RetrieveDevices(client, cancellationToken);
  }

  private async Task UseStaticBuilder(CancellationToken cancellationToken)
  {
    _logger.LogInformation("Retrieving devices using PyTrainApiClientBuilder.");
