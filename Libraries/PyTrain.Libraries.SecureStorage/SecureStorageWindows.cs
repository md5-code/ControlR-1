...
namespace PyTrain.Libraries.SecureStorage;
...
internal class SecureStorageWindows : ISecureStorage
{
    ...
    public SecureStorageWindows(ILogger<SecureStorageWindows> logger, IOptions<SecureStorageOptions> options)
    {
        ...
        var pyTrainPath = Path.Combine(appDataPath, _options.ServiceName);
        Directory.CreateDirectory(pyTrainPath);
        _storagePath = Path.Combine(pyTrainPath, $"{_options.ServiceName.ToLowerInvariant()}_secure_storage.dat");
