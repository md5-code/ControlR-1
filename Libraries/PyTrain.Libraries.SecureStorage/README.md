# PyTrain.Libraries.SecureStorage

A cross-platform secure storage library for PyTrain, providing a simple key/value API similar to MAUI's SecureStorage.
...
- Stores encrypted data in `%LOCALAPPDATA%\PyTrain\`
...
- Stores encrypted data in `~/.config/pytrain/` (user) or `/etc/pytrain/` (root)
...
```csharp
using PyTrain.Libraries.SecureStorage;
...
This library is part of the PyTrain project and follows the same license.
