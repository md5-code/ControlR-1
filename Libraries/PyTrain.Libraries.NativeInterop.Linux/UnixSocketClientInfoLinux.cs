...
namespace PyTrain.Libraries.NativeInterop.Linux;
...
public static class UnixSocketClientInfoLinux
{
  ...
  public static Result<IpcClientCredentials> GetClientCredentials(SafeHandle socketHandle)
  {
    try
    {
      if (socketHandle.IsInvalid || socketHandle.IsClosed)
      {
        return Result.Fail<IpcClientCredentials>("Socket handle is invalid or closed.");
      }
      ...
      if (result != 0)
      {
        ...
        return Result.Fail<IpcClientCredentials>($"Failed to get peer credentials. Error: {error}");
      }

      if (ucred.pid == 0)
      {
        return Result.Fail<IpcClientCredentials>("Client process ID is 0.");
      }

      var executablePath = GetProcessExecutablePath(ucred.pid);
      if (string.IsNullOrWhiteSpace(executablePath))
      {
        return Result.Fail<IpcClientCredentials>($"Failed to get executable path for process {ucred.pid}.");
      }

      return Result.Ok(new IpcClientCredentials(ucred.pid, executablePath));
    }
    catch (Exception ex)
    {
      return Result.Fail<IpcClientCredentials>($"Error getting client credentials: {ex.Message}");
    }
  }
