...
namespace PyTrain.Libraries.Shared.Services.Logging;

public class FileLogger(
  string componentVersion,
  string categoryName,
  Func<string> logPathFactory,
  TimeSpan logRetention) : ILogger
{
    ...
    Func<TState, Exception?, string> formatter)
  {
    try
    {
      var message = FormatLogEntry(logLevel, categoryName, $"{state}", exception, [.. _scopeStack]);
  ...
  private void CleanupLogs()
  {
    ...
    var logFiles = Directory.GetFiles(Path.GetDirectoryName(logPathFactory())!)
      .Select(x => new FileInfo(x))
      .Where(x => DateTime.Now - x.CreationTime > logRetention);
