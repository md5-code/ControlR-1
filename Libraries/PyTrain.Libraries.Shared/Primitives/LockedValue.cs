...
public class LockedValue<T>(T? value) : IDisposable where T : class?
{
  ...
  public LockedValueToken<T?> Lock()
  {
    _lock.Enter();
    return new LockedValueToken<T?>(() => _value, v => _value = v, _lock.Exit);
  }
  ...
  public LockedValueToken<T?>? TryLock(TimeSpan timeout)
  {
  ...
  public LockedValueToken<T?>? TryLock()
  {
    if (_lock.TryEnter())
    {
      return new LockedValueToken<T?>(() => _value, SetValue, _lock.Exit);
    }
    return null;
  }
