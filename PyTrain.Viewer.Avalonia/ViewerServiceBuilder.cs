...
/// <summary>
///   Builds service providers for individual <see cref="PyTrainViewer"/> instances.
...
public static class ViewerServiceBuilder
{
  ...
  /// <summary>
  ///   Call this method at application startup, before any <see cref="PyTrainViewer"/> instances are created,
  ...
  internal static IServiceProvider BuildServiceProvider(PyTrainViewerOptions viewerOptions, Guid instanceId, IClipboard clipboard)
  {
    ...
    // Register options.
    services
     .AddOptions<PyTrainViewerOptions>()
      .Configure(opts =>
      {
    ...
    services.AddPyTrainSnackbar();
    services.AddPyTrainDialogs();
