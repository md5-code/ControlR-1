...
public partial class RemoteControlViewModel : ViewModelBase<RemoteControlView>, IRemoteControlViewModel
{
  ...
  private readonly IOptions<PyTrainViewerOptions> _viewerOptions;
  ...
  public RemoteControlViewModel(
    ...
    IOptions<PyTrainViewerOptions> viewerOptions,
  ...
  {
    var result = await _pytrainApi.UserPreferences.GetUserPreferences();
    if (!result.IsSuccess)
    {
      return;
    }
  ...
  private async Task<bool> StartRemoteControl(DesktopSession desktopSession, bool quiet)
  {
    try
    {
      ...
      await _remoteControlStream.Connect(
        ...
        wsOptions =>
        {
          wsOptions.SetRequestHeader(PyTrainViewerOptions.PersonalAccessTokenHeaderName, _viewerOptions.Value.PersonalAccessToken);
        },
        cts.Token);
