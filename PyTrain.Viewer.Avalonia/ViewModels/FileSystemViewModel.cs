...
using PyTrain.Libraries.Avalonia.Controls.Snackbar;
using PyTrain.Libraries.Shared.Helpers;
using PyTrain.Libraries.Shared.IO;
using PyTrain.Libraries.Shared.Primitives;
using PyTrain.Viewer.Avalonia.ViewModels.Dialogs;

namespace PyTrain.Viewer.Avalonia.ViewModels;
...
public partial class FileSystemViewModel : ViewModelBase<FileSystemView>, IFileSystemViewModel
{
  ...
  private readonly IPyTrainApi _pytrainApi;
  ...
  public async Task CreateNewFolder()
  {
    ...
    try
    {
      var result = await _pytrainApi.DeviceFileSystem.CreateDirectory(new CreateDirectoryRequestDto(_deviceState.CurrentDevice.Id, SelectedPath!, folderName));
      if (!result.IsSuccess)
      {
  ...
  public async Task DeleteSelection()
  {
    ...
    try
    {
      ...
      {
        var result = await _pytrainApi.DeviceFileSystem.DeleteFile(new FileDeleteRequestDto(_deviceState.CurrentDevice.Id, item.FullPath, item.IsDirectory));
        if (!result.IsSuccess)
        {
  ...
  public async Task DownloadSelection(IStorageFile destinationFile)
  {
    ...
    try
    {
      ...
      {
        var result = await _pytrainApi.DeviceFileSystem.DownloadFile(_deviceState.CurrentDevice.Id, SelectedItems[0].FullPath);
      ...
      else
      {
        ...
        var result = await _pytrainApi.DeviceFileSystem.DownloadArchive(_deviceState.CurrentDevice.Id, request);
        if (!result.IsSuccess || result.Value is null)
        {
          _snackbar.Add(string.Format(Assets.Resources.FileSystem_DownloadFailureDetailsMessage, result.Reason), SnackbarSeverity.Error);
          return;
  ...
  public async Task UploadFiles(IReadOnlyList<IStorageFile> files)
  {
    ...
    foreach (var file in files)
    {
      try
      {
        ...
        var result = await _pytrainApi.DeviceFileSystem.UploadFile(_deviceState.CurrentDevice.Id, fileStream, file.Name, SelectedPath!, overwrite);
        if (!result.IsSuccess)
        {
  ...
  public async Task UpOneLevel()
  {
    ...
    try
    {
      var result = await _pytrainApi.DeviceFileSystem.GetPathSegments(new GetPathSegmentsRequestDto(_deviceState.CurrentDevice.Id, SelectedPath!));
  ...
  private async Task<bool> BuildTreeToPath(string targetPath)
  {
    try
    {
      var result = await _pytrainApi.DeviceFileSystem.GetPathSegments(new GetPathSegmentsRequestDto(_deviceState.CurrentDevice.Id, targetPath));
      if (!result.IsSuccess || result.Value is null)
      {
  ...
  {
    var result = await _pytrainApi.UserServerSettings.GetFileUploadMaxSize();
    if (!result.IsSuccess)
    {
  ...
  private async Task LoadDirectoryContents(string directoryPath)
  {
    try
    {
      ...
      var result = await _pytrainApi.DeviceFileSystem.GetDirectoryContents(new GetDirectoryContentsRequestDto(_deviceState.CurrentDevice.Id, directoryPath));
      if (!result.IsSuccess || result.Value is null)
      {
  ...
  private async Task LoadRootDrives()
  {
    try
    {
      IsLoading = true;
      RootItems.Clear();
      var result = await _pytrainApi.DeviceFileSystem.GetRootDrives(new GetRootDrivesRequestDto(_deviceState.CurrentDevice.Id));
      if (!result.IsSuccess || result.Value is null)
      {
  ...
  private async Task LoadTreeItemChildrenCore(FileSystemTreeItemViewModel item, bool forceReload)
  {
    ...
    try
    {
      var result = await _pytrainApi.DeviceFileSystem.GetSubdirectories(new GetSubdirectoriesRequestDto(_deviceState.CurrentDevice.Id, item.FullPath));
      if (!result.IsSuccess || result.Value is null)
      {
        return;
      }
