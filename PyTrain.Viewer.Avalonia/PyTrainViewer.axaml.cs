...
using PyTrain.Viewer.Avalonia.ViewModels.Fakes;
using PyTrain.Viewer.Avalonia.Services;
using PyTrain.Viewer.Avalonia.Services.Navigation;
using System.ComponentModel;

namespace PyTrain.Viewer.Avalonia;

public partial class PyTrainViewer : UserControl
{
  public static readonly StyledProperty<Guid> InstanceIdProperty =
    AvaloniaProperty.Register<PyTrainViewer, Guid>(nameof(InstanceId));
  public static readonly StyledProperty<PyTrainViewerOptions?> OptionsProperty =
    AvaloniaProperty.Register<PyTrainViewer, PyTrainViewerOptions?>(nameof(Options));
  public static readonly StyledProperty<ViewerPage> PageProperty =
    AvaloniaProperty.Register<PyTrainViewer, ViewerPage>(nameof(Page), ViewerPage.None);
  ...
  public PyTrainViewerOptions? Options
  {
  ...
  public ViewerInstanceInfo GetInstanceInfo()
  {
    ...
    {
      throw new InvalidOperationException(Assets.Resources.PyTrainViewer_ServiceProviderNotInitialized);
    }
  ...
  protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
  {
    ...
    if (Options is null)
    {
      SetErrorContent(Assets.Resources.PyTrainViewer_OptionsNotSet);
      return;
    }

    if (Options.BaseUrl is null)
    {
      SetErrorContent(Assets.Resources.PyTrainViewer_BaseUrlRequired);
      return;
    }

    if (Options.DeviceId == Guid.Empty)
    {
      SetErrorContent(Assets.Resources.PyTrainViewer_DeviceIdRequired);
      return;
    }

    if (string.IsNullOrWhiteSpace(Options.PersonalAccessToken))
    {
      SetErrorContent(Assets.Resources.PyTrainViewer_PatRequired);
      return;
    }
  ...
  private void InitializeServices()
  {
    ...
    try
    {
      ...
      {
        SetErrorContent(Assets.Resources.PyTrainViewer_TopLevelMissing);
        return;
      }

      if (topLevel.Clipboard is not { } clipboard)
      {
        SetErrorContent(Assets.Resources.PyTrainViewer_ClipboardMissing);
        return;
      }
    ...
    catch (Exception ex)
    {
      ...
      SetErrorContent(string.Format(Assets.Resources.PyTrainViewer_InitializationError, ex.Message));
    }
    finally
    {
      _isInitialized = true;
