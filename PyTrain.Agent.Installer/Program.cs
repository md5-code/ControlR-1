using PyTrain.Agent.Installer.Services;
...
using PyTrain.Libraries.Shared.DataValidation;
...
const string RootDescription = "PyTrain agent installer.";
const string InstallCommandName = "install";
const string UninstallCommandName = "uninstall";
const string InstallCommandDescription = "Install the PyTrain agent bundle.";
const string UninstallCommandDescription = "Uninstall the PyTrain agent bundle.";
...
const string InstanceIdDescription = "An instance ID for this agent installation, which allows multiple agent installations.  This is typically the server origin (e.g. 'example.internal.local').";
...
const string TempDirectoryPrefix = "pytrain-install-";
const string TempBundleFileName = "PyTrain.Agent.bundle.zip";
...
static async Task<int> RunInstall(AgentInstallRequest request, string? instanceId)
{
  ...
  var logger = host.Services.GetRequiredService<ILoggerFactory>().CreateLogger("PyTrain.Agent.Installer");
  ...
  try
  {
    var api = host.Services.GetRequiredService<IPyTrainApi>();
    ...
    logger.LogInformation("PyTrain Agent Installer started.");
...
static IHost CreateInstallerHost(string? instanceId, Uri? serverUri)
{
  ...
  builder.AddPyTrainInstallerServices(instanceId, serverUri, loadAppSettings: true);
