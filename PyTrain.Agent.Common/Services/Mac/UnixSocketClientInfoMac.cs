  ...
  public static Result<IpcClientCredentials> GetClientCredentials(SafeHandle socketHandle)
  {
    try
    {
      ...
      if (result != 0 || pid == 0)
      {
        ...
        return Result.Fail<IpcClientCredentials>($"Failed to get peer process ID using LOCAL_PEERPID. Error: {error}");
      }

      var executablePath = GetProcessExecutablePath(pid);
      if (string.IsNullOrWhiteSpace(executablePath))
      {
        return Result.Fail<IpcClientCredentials>($"Failed to get executable path for process {pid}.");
      }

      return Result.Ok(new IpcClientCredentials(pid, executablePath));
    }
    catch (Exception ex)
    {
