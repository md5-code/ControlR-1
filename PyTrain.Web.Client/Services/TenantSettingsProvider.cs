  ...
  ILogger<TenantSettingsProvider> logger) : ITenantSettingsProvider
{
  ...
  public async Task<TenantSettingsDto> GetSettings()
  {
    ...
    var getResult = await _pytrainApi.TenantSettings.GetTenantSettings();
  ...
  private async Task SetSetting<T>(string settingName, T newValue)
  {
    try
    {
      if (newValue is null)
      {
        var deleteResult = await _pytrainApi.TenantSettings.DeleteTenantSetting(settingName);
      ...
      var setResult = await _pytrainApi.TenantSettings.SetTenantSetting(request);
