...
public partial class UsersTabContent : ComponentBase, IDisposable
{
  ...
  private async Task DeleteSelectedUser()
  {
    ...
    try
    {
      ...
      var deleteResult = await PyTrainApi.Users.DeleteUser(_selectedUser.Id);
  ...
  private async Task SetUserRole(bool isToggled, UserResponseDto user, RoleViewModel role)
  {
    try
    {
      if (isToggled)
      {
        ...
        var addResult = await PyTrainApi.UserRoles.AddUserRole(addRequest);
      ...
      {
        var removeResult = await PyTrainApi.UserRoles.RemoveUserRole(user.Id, role.Id);
  ...
  private async Task SetUserTag(bool isToggled, UserResponseDto user, TagViewModel tag)
  {
    try
    {
      ...
      {
        var removeResult = await PyTrainApi.UserTags.RemoveUserTag(user.Id, tag.Id);
        if (!removeResult.IsSuccess)
        {
          Snackbar.Add(removeResult.Reason, Severity.Error);
          return;
