...
using PyTrain.Web.Client.DataValidation;
using Microsoft.AspNetCore.Components.Web;

namespace PyTrain.Web.Client.Components.Permissions;
...
public partial class TagsTabContent : ComponentBase, IDisposable
{
  ...
  private async Task CreateTag()
  {
    ...
    var createResult = await PyTrainApi.Tags.CreateTag(createRequest);
  ...
  private async Task DeleteSelectedTag()
  {
    ...
    var deleteResult = await PyTrainApi.Tags.DeleteTag(_selectedTag.Id);
  ...
  private async Task RenameSelectedTag()
  {
    ...
    var renameResult = await PyTrainApi.Tags.RenameTag(renameRequest);
  ...
  private async Task SetDeviceTag(bool isToggled, TagViewModel tag, Guid deviceId)
  {
    try
    {
      if (isToggled)
      {
        ...
        var addResult = await PyTrainApi.DeviceTags.AddDeviceTag(addRequest);
      ...
      {
        var removeResult = await PyTrainApi.DeviceTags.RemoveDeviceTag(deviceId, tag.Id);
  ...
  private async Task SetUserTag(bool isToggled, TagViewModel tag, Guid userId)
  {
    try
    {
      if (isToggled)
      {
        ...
        var addResult = await PyTrainApi.UserTags.AddUserTag(addRequest);
        if (!addResult.IsSuccess)
        {
          Snackbar.Add(addResult.Reason, Severity.Error);
          return;
      ...
      {
        var removeResult = await PyTrainApi.UserTags.RemoveUserTag(userId, tag.Id);
        if (!removeResult.IsSuccess)
        {
          Snackbar.Add(removeResult.Reason, Severity.Error);
          return;
