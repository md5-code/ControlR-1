...
public partial class PersonalAccessTokens
{
  ...
  private async Task CreatePersonalAccessToken()
  {
    ...
    try
    {
      ...
      var result = await PyTrainApi.PersonalAccessTokens.CreatePersonalAccessToken(request);
  ...
  private async Task DeletePersonalAccessToken(PersonalAccessTokenDto personalAccessToken)
  {
    ...
    if (confirmed == true)
    {
      try
      {
        var result = await PyTrainApi.PersonalAccessTokens.DeletePersonalAccessToken(personalAccessToken.Id);
        if (result.IsSuccess)
        {
          await LoadPersonalAccessTokens();
  ...
  private async Task LoadPersonalAccessTokens()
  {
    ...
    try
    {
      var result = await PyTrainApi.PersonalAccessTokens.GetPersonalAccessTokens();
      if (result.IsSuccess)
      {
  ...
  private async Task RenamePersonalAccessToken(PersonalAccessTokenDto personalAccessToken)
  {
    ...
    try
    {
      ...
      var updateResult = await PyTrainApi.PersonalAccessTokens.UpdatePersonalAccessToken(personalAccessToken.Id, updateRequest);
