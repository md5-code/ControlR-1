...
public partial class InstallerKeys
{
  ...
  private async Task DeleteKey(AgentInstallerKeyDto key)
  {
    try
    {
      ...
      var apiResult = await PyTrainApi.InstallerKeys.DeleteInstallerKey(key.Id);
  ...
  private async Task RenameKey(AgentInstallerKeyDto key)
  {
    try
    {
      ...
      var result = await PyTrainApi.InstallerKeys.RenameInstallerKey(dto);
  ...
  private async Task ShowUsages(AgentInstallerKeyDto key)
  {
    try
    {
      var result = await PyTrainApi.InstallerKeys.GetInstallerKeyUsages(key.Id);
      if (!result.IsSuccess)
      {
