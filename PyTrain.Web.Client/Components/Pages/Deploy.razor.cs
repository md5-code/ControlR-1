using PyTrain.Web.Client.Extensions;
using Microsoft.AspNetCore.Components;
using Microsoft.AspNetCore.Components.Authorization;

namespace PyTrain.Web.Client.Components.Pages;
...
public partial class Deploy
{
  ...
  [Inject]
  public required IPyTrainApi PyTrainApi { get; init; }
  [Inject]
  ...
  private string LinuxDeployScript
  {
    get
    {
      ...
      return
        $"sudo rm -f /tmp/PyTrain.Agent.Installer && " +
        $"sudo curl -o /tmp/PyTrain.Agent.Installer {downloadUri} && " +
        $"sudo chmod +x /tmp/PyTrain.Agent.Installer && " +
        $"sudo /tmp/PyTrain.Agent.Installer install {GetCommonArgs()}";
    }
  }
  ...
  private string WindowsX64DeployScript
  {
    get
    {
      ...
      return "$ProgressPreference = 'SilentlyContinue'; " +
             $"Invoke-WebRequest -Uri \"{downloadUri}\" -OutFile \"$env:TEMP/PyTrain.Agent.Installer.exe\" -UseBasicParsing; " +
             $"Start-Process -FilePath \"$env:TEMP/PyTrain.Agent.Installer.exe\" -ArgumentList \"install {GetCommonArgs()}\" -Verb RunAs;";
    }
  }
  ...
  protected override async Task OnInitializedAsync()
  {
    ...
    var result = await PyTrainApi.UserTags.GetAllowedTags();
    if (result.IsSuccess)
    {
      _tags = result.Value;
    }
  ...
  private async Task GeneratePersistentKey()
  {
    ...
    var createResult = await PyTrainApi.InstallerKeys.CreateInstallerKey(dto);
  ...
  private async Task ToggleKeyMode(bool useExisting)
  {
    ...
    {
      var result = await PyTrainApi.InstallerKeys.GetAllInstallerKeys();
      if (result.IsSuccess)
      {
