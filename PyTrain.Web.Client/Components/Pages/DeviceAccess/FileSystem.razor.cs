...
using PyTrain.Web.Client.Components.FileSystem;
...
public partial class FileSystem : JsInteropableComponent
{
  ...
  [Inject]
  public required IPyTrainApi PyTrainApi { get; set; }
  ...
  private async Task<bool> BuildTreeToPath(string targetPath)
  {
    try
    {
      ...
      var pathSegmentsResult = await PyTrainApi.DeviceFileSystem.GetPathSegments(pathSegmentsRequest);
  ...
  private async Task DeleteSingleItem(FileSystemEntryViewModel item)
  {
    try
    {
      ...
      var result = await PyTrainApi.DeviceFileSystem.DeleteFile(request);
  ...
  {
    var getMaxSizeResult = await PyTrainApi.UserServerSettings.GetFileUploadMaxSize();
  ...
  private async Task LoadDirectoryContents(string directoryPath)
  {
    try
    {
      ...
      var result = await PyTrainApi.DeviceFileSystem.GetDirectoryContents(request);
  ...
  private async Task LoadRootDrives()
  {
    try
    {
      ...
      var result = await PyTrainApi.DeviceFileSystem.GetRootDrives(request);
      if (result is { IsSuccess: true, Value: not null })
      {
  ...
  private async Task<IReadOnlyCollection<TreeItemData<string>>> LoadServerData(string? parentValue)
  {
    try
    {
      ...
      var result = await PyTrainApi.DeviceFileSystem.GetSubdirectories(request);
      if (result is { IsSuccess: true, Value: not null })
      {
  ...
  private async Task OnNewFolderClick()
  {
    ...
    try
    {
      ...
      var result = await PyTrainApi.DeviceFileSystem.CreateDirectory(request);
      if (result.IsSuccess)
      {
