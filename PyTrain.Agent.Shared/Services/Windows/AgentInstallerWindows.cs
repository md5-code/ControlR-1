...
internal class AgentInstallerWindows(
  ...
  : AgentInstallerBase(fileSystem, fileSystemPathProvider, pytrainApi, deviceDataGenerator, optionsAccessor, processes, systemEnvironment, appOptions, logger), IAgentInstaller
{
  ...
  public async Task Install(AgentInstallRequest request)
  {
    ...
    {
      Logger.LogError(ex, "Error while installing the PyTrain service.");
    }
    finally
    {
  ...
  public async Task Uninstall()
  {
    ...
    {
      Logger.LogError(ex, "Error while uninstalling the PyTrain service.");
    }
    finally
    {
      _installLock.Release();
  ...
  private void CreateUninstallKey()
  {
    ...
    using var pytrainKey = baseKey.CreateSubKey(GetUninstallKeyPath(), true);
    pytrainKey.SetValue("DisplayIcon", Path.Combine(installDir, fileName));
    pytrainKey.SetValue("DisplayName", displayName);
    pytrainKey.SetValue("DisplayVersion", version.FileVersion ?? "0.0.0");
    pytrainKey.SetValue("InstallDate", DateTime.Now.ToString("yyyyMMdd"));
    pytrainKey.SetValue("Publisher", "md5-hash");
    pytrainKey.SetValue("VersionMajor", $"{version.FileMajorPart}", RegistryValueKind.DWord);
    pytrainKey.SetValue("VersionMinor", $"{version.FileMinorPart}", RegistryValueKind.DWord);
    pytrainKey.SetValue("UninstallString", uninstallCommand);
    pytrainKey.SetValue("QuietUninstallString", uninstallCommand);
  }

  private string GetInstallDirectory()
  {
    var rootDirectory = _systemEnvironment.IsDebug
      ? Path.Combine(Path.GetTempPath(), "PyTrain", "Install")
      : Path.Combine(Path.GetPathRoot(Environment.SystemDirectory) ?? "C:\\", "Program Files", "PyTrain");
  ...
  private bool IsRunningFromAppDir()
  {
    ...
    var dest = Path.Combine(Path.GetTempPath(), $"PyTrain_{Guid.NewGuid()}.exe");
