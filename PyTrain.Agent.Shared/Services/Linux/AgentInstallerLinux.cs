...
internal class AgentInstallerLinux(
  ...
  : AgentInstallerBase(fileSystem, fileSystemPathProvider, pytrainApi, deviceDataGenerator, optionsAcccessor, processManager, systemEnvironment, appOptions, logger), IAgentInstaller
{
  ...
  private string GetDesktopServiceFilePath()
  {
    ...
    {
      return "/etc/systemd/user/pytrain.desktop.service";
    }

    return $"/etc/systemd/user/pytrain.desktop-{instanceOptions.Value.InstanceId}.service";
  }
  ...
  {
    return GetInstanceInstallDirectory("/usr/local/bin/PyTrain", instanceOptions.Value.InstanceId);
  }
  ...
  private string GetServiceFilePath()
  {
    if (string.IsNullOrWhiteSpace(instanceOptions.Value.InstanceId))
    {
      return "/etc/systemd/system/pytrain.agent.service";
    }

    return $"/etc/systemd/system/pytrain.agent-{instanceOptions.Value.InstanceId}.service";
  }
  ...
  private void SetExecutablePermissions(string installDirectory)
  {
    ...
    {
      Path.Combine(installDirectory, "PyTrain.Agent"),
      Path.Combine(installDirectory, "DesktopClient", "PyTrain.DesktopClient")
    })
    {
