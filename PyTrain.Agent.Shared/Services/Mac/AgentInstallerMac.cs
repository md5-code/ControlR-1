...
internal class AgentInstallerMac(
  ...
  : AgentInstallerBase(fileSystem, fileSystemPathProvider, pytrainApi, deviceDataGenerator, optionsAccessor, processManager, systemEnvironment, appOptions, logger), IAgentInstaller
{
  private const string MacAgentInstallDirectory = "/Library/Application Support/PyTrain";
  private const string MacBundleStateDirectory = "/Library/Application Support/PyTrain";
  private const string MacInstallerTempDirectory = "/tmp/PyTrain_Update";
  ...
  public async Task Install(AgentInstallRequest request)
  {
    ...
    try
    {
      ...
      try
      {
        var tempAppBundlePath = Path.Combine(tempExtractDirectory, "PyTrain.app");
        await retryer.Retry(
          async () => {
    ...
    {
      _logger.LogError(ex, "Error while installing the PyTrain service.");
    }
    finally
    {
  ...
  public async Task Uninstall()
  {
    ...
    {
      _logger.LogError(ex, "Error while uninstalling the PyTrain service.");
    }
    finally
    {
      _installLock.Release();
  ...
  internal async Task<string> GetLaunchAgentFile()
  {
    var serviceName = string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId)
      ? "app.pytrain.desktop"
      : $"app.pytrain.desktop.{_instanceOptions.Value.InstanceId}";
  ...
  private string GetAgentServiceName()
  {
    return string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId)
      ? "app.pytrain.agent"
      : $"app.pytrain.agent.{_instanceOptions.Value.InstanceId}";
  }
  ...
  private string GetInstallerDaemonFilePath()
  {
    if (string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId))
    {
      return "/Library/LaunchDaemons/app.pytrain.agent.installer.plist";
    }

    return $"/Library/LaunchDaemons/app.pytrain.agent.installer.{_instanceOptions.Value.InstanceId}.plist";
  }
  ...
  private string GetInstallerDaemonServiceName()
  {
    if (string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId))
    {
      return "app.pytrain.agent.installer";
    }

    return $"app.pytrain.agent.installer.{_instanceOptions.Value.InstanceId}";
  }
  ...
  private string GetLaunchAgentFilePath()
  {
    if (string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId))
    {
      return "/Library/LaunchAgents/app.pytrain.desktop.plist";
    }

    return $"/Library/LaunchAgents/app.pytrain.desktop.{_instanceOptions.Value.InstanceId}.plist";
  }
  ...
  private string GetLaunchDaemonFilePath()
  {
    if (string.IsNullOrWhiteSpace(_instanceOptions.Value.InstanceId))
    {
      return "/Library/LaunchDaemons/app.pytrain.agent.plist";
    }

    return $"/Library/LaunchDaemons/app.pytrain.agent.{_instanceOptions.Value.InstanceId}.plist";
  }
  ...
  private void SetExecutablePermissions(string appBundlePath)
  {
    ...
    {
      Path.Combine(appBundlePath, "Contents", "MacOS", "PyTrain.DesktopClient"),
      Path.Combine(appBundlePath, "Contents", "Library", "LaunchServices", "PyTrain.Agent")
    })
    {
